...
export const getProdutosPublicos = async (req, res) => {
    try {
        ...
        // Captura a palavra que a IA enviou lá do n8n
        const { busca } = req.query; 

        if (!tenantId) return res.status(401).json({ error: "Loja não identificada na chave de API." });

        // Monta o filtro básico
        const regrasBusca = { 
            id_tenant: tenantId, 
            ativo: true 
            // active_ecommerce: true // (Comentado para permitir que o bot venda itens do PDV/Suco)
        };

        // Se a IA pesquisou uma palavra (ex: "suco"), adiciona no filtro do banco!
        if (busca && busca.trim() !== '') {
            regrasBusca.nome = {
                contains: busca
                // Nota: O Prisma com MySQL geralmente já faz a busca ignorando maiúsculas/minúsculas.
            };
        }
